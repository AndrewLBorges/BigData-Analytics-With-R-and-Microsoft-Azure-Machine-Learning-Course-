...
# Podemos ver que as variáveis Sex, Age e Pclass são as variáveis mais significantes
summary(log.model)
...
final.log.modelo <- glm(formula = Survived ~ ., family = binomial(link = 'logit'), data = dados_treino_final)

# REsumo
summary(final.log.modelo)

# Prevendo a acurácia
fitted.probabilities <- predict(final.log.modelo, newdata = dados_teste_final, type = 'response')

# Calculando os valores
fitted.results <- ifelse(fitted.probabilities > 0.5, 1, 0)

# Conseguimos quase 80% de acurácia
misClassificError <- mean(fitted.results != dados_teste_final$Survived)
print(paste('Acuracia', 1-misClassificError))
