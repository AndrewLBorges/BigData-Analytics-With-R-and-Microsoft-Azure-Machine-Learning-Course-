...
# H0 (Hipótese Nula) - Não há difrença significativa entre as médias dos 2 grupos
# (logo o tipo de suplemento não tem impacto no crescimento dos dente)
...
# 5 - A variãncia residual é homogênea (princípio da homocedasticidade).
...
# H0 - As médias de dados extraídos de uma população normalmente distribuida tem a mesma variância.
# H1 - AS médias de dados extraídos de uma população normalmente distribuida não tem a mesma variãncia
?var.test
var.test(len ~ supp, data=ToothGrowth)

# O valor-p é maior que 0.05. Falhamos em rejeitar a H0.
# Não há diferença significativa entre as variãncias dos 2 grupos.
...
# O valor-p do primeiro grupo é menor que 0.05. Rejeitamos H0.
...
df_0J = ToothGrowth$len[ToothGrowth$supp == 'OJ']
df_VC = ToothGrowth$len[ToothGrowth$supp == "VC"]

t.test(df_0J, df_VC, paired = FALSE, var.equal = FALSE, conf.level = 0.95)

# O valor-p do tete é 0.06063, logo, maior que 0.05. Falhamos em rejeirar a H0.
# Podemos concluir que os 2 grupos não tem diferença significativa.
# Ou seja, não há diferença significativa entre os tipos de suplementos aplicados para o crescimento dos dentes
# dos Porcos da Guiné. Compare com o resultado do Boxplot.

# Solução 1 não é válida para esse tipo de problema pois os dados não tatisfazem 
# uma das suposições
...
# 2 - Cada amostra de grupo é extraida de uma população normalmente distribuida.
...
# H1: Os dados não são normalmente distribuidos
...
# As 3 amostras são norlmalmente distribuidas
