...
# Carrega o arquivo coma descrição dos testes
...
ui <- navbarPage(
  ...
  # Cria instãncia do shinyjs
  shinyjs::useShinyjs(),
  
  # Tema de cores o dashboard
...
server <- function(input, output, session) {
  ...
  # Vetor dos testes que requerem a média amostra
  output$samplemean <- renderUI({
    samplemean <- c("Teste t Para Uma Amostra", "Teste de Wilcoxon Signed Rank", "Teste t Para Duas Amostras")
    
    # Se o teste selecionado estiver na lista anterior, mostrar a caixa solicitando a média da amostra
...
# Valida as amostras
  ...
  stat_test <- eventReactive(input$generate, {
    ...
    test_result_tidy <- test_result %>%
      mutate(result = ifelse(p.value <= 0.05, "Estatísticamente Significante, Rejeitamos H0",
                                              "Estatísticamente Insignificante, Falhamos em Rejeitar a H0")) %>%
      t() %>%
  ...
  # Prepara os ados para o histograma
