...
# A variável 4 é categorica
table(dataset$prioridade)

# Vamos converte a variável alvo para o tipo fator
...
# Plot da importãncia de cada varivável para prever a variável alvo
?reorder
df_var_contrib %>%
...
ggplot(df_var_contrib, aes(x = shap_value, y = reorder(feature, shap_importance))) +
  ggbeeswarm::geom_quasirandom(varwidth = TRUE, size = 0.9, alpha = 0.5, width = 0.15) +
