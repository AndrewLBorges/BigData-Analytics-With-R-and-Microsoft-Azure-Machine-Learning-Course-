...
# Verificano ocorrência de valores NA
colSums(is.na(dados))
...
# 7- Qúal o gasto total com internações hospitalares por genero?
...
# 8- QUal a média de gasto total com internações hospitalares por raça do paciente?
gasto_medio_race <- sqldf("
                          SELECT RACE as Raca,
                          AVG(TOTCHG) as Media_Gasto_Total
                          FROM dados
                          GROUP BY RACE")
gasto_medio_race
...
tapply(gasto_total_baseado_idade$TOTCHG, gasto_total_baseado_idade$AGE, FUN = sum)
which.max(tapply(gasto_total_baseado_idade$TOTCHG, gasto_total_baseado_idade$AGE, FUN = sum))
...
# Pergunta 3:
# Qual grupo baseado em diagnóstico (Aprdrg) tem maior gasto total no hospital?
...
# Pergunta 5:
# A combinação de idade e gênero dos paciente influencia no gasto total
...
summary(dados$AGE)
summary(dados$FEMALE)
summary(as.factor(dados$AGE))
summary(as.factor(dados$FEMALE))
...
# desejamos descobrir se o tempo de permanência pode ser visto a partir de
# idade, gênero e raça.
...
# Valor-p maior que 0.05 em todos os casos, logo, falhamos em rejeitar H0.
...
# Variáveis independentes: AGE, FEMALE, LOS, RACE, APRDRG
...
# Como podemos observar a partir dos valaores dos coeficientes, as variáveis idade,
...
# As 3 variáveis te malta significância, mas APRDRG tem valor t negativo.
# Vamos removê-la e criar outra versão do modelo.
...
# Logo, o modelo modelo_lo_3var paree ser o melhor e o usaremos para nossa conclusão.
summary(modelo_lr_3var)
