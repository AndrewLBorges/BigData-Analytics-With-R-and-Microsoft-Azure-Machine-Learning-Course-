...
incomplete_cases <- sum(!complete.cases(dados))

# QUal o percentual de dados incompletos?
percentual <- (incomplete_cases / complete_cases) * 100
percentual
...
list_of_countries_with_na <- unique(dados$NomePais)
list_of_countries_with_na
...
length(list_of_countries_with_na)
length(list_of_countries_without_na)

# Verificando a diferença antes e depois de remover valores NA
setdiff(list_of_countries_with_na, list_of_countries_without_na)

# Remove os objetos
rm(list_of_countries_with_na)
rm(list_of_countries_with_na)
...
dados_por_ano <- dados[dados$Ano != 2005 & dados$Ano != 2006 & dados$Ano != 2007 & dados$Ano != 2020, ]

# Número de registros por ano
table(dados_por_ano$Ano)
...
# às empresas e ao governo do país?
...
# Pergunta 5
# Pessoas geenrosas são mais felizes?
